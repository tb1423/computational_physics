...
for _t in t_space:
    ...
    rho_pdf = to.rho_particular_3d_num(x_space,y_space,z_space,_t)

    ## Generate local energy function
    E_l = to.local_energy_3d(x_space,y_space,z_space,_t)
    ...
    H_mean.append(np.sum(np.array(loc_enrg)) / np.size(loc_enrg))
    d_th_H.append(to.del_th_H(R_i, loc_enrg,_t,t_space))

    print(f'<H> = {np.sum(np.array(loc_enrg)) / np.size(loc_enrg)}, T = {_t}')

plt.scatter(t_space, H_mean,marker='x')
plt.xlabel('theta')
plt.ylabel('<H>')
plt.savefig('fig/tspace.png')
plt.show()
plt.scatter(t_space, d_th_H,marker='x')
plt.xlabel('theta')
plt.ylabel('d<H>/dth')
plt.savefig('fig/d_dtspace.png')
plt.show()
